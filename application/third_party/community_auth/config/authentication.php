...
/*
-------------------------------------------------------------------------
	This blocked by Ramesh for getting the roles from database*
-------------------------------------------------------------------------

|	The similer declared in hooks 
|	File name "hooks/auth_constants.php"
|	Function name "auth_constants"

*/

	$CI =& get_instance();
	$roleData = $CI->mcommon->records_all('app_roles', '');

	$levels_and_roles = array(
								'1' => 'user',
								'9' => 'admin'
							);
	...
	{
		$levels_and_roles[$row->role_id]	=	$row->role_name;
	}

	$CI->config->set_item('levels_and_roles', $levels_and_roles);
...
/*
| -----------------------------------------------------------------
|				ADD USER PROFILE DATA TO AUTH FUNCTIONS
| -----------------------------------------------------------------
|
*/

$config['add_user_profile_to_auth_functions'] = TRUE;
...
$config['max_allowed_attempts'] = 55;
