...
class Auth_model extends MY_Model {
	...
	public function get_auth_data( $user_string )
	{
		...
		if( $query->num_rows() == 1 )
		{
			...
			return (object) array_merge( $acl, $row,$auth_profile,$auth_address);
	...
	public function check_login_status( $user_id, $login_time )
	{
		...
		if( $query->num_rows() == 1 )
		{
			...
			// ACL is added
			$acl = $this->add_acl_to_auth_data($row['user_id']);
			$profile_merged = array_merge($row,$this->add_profile_data($row['user_id']));
			$address_merged = array_merge($profile_merged,$this->add_address_data($row['user_id']));
			$contact_merged =array_merge($address_merged,$this->add_contact_data($row['user_id']));

			return (object) array_merge($contact_merged, $acl);
		}
	...
	public function add_profile_data($user_id)
	{
		$profile_data = [];


		$query = $this->db->select('*')
			->from( $this->db_table('user_profile_table') )
	...
	public function add_address_data($user_id)
	{
		$address_data = [];


		$query = $this->db->select('*')
			->from( $this->db_table('user_address_table') )
			->where( 'LOWER(user_id) =', strtolower( $user_id ) )
			->get();
		...
		return ['address_data' => $address_data];
	}


	public function add_contact_data($user_id)
	{
		$contact_data = [];


		$query = $this->db->select('*')
			->from( $this->db_table('user_phone_table') )
			->where( 'LOWER(user_id) =', strtolower( $user_id ) )
			->get();

		if( $query->num_rows() != 0 )
		{
			$contact_data = $query->result_array();
